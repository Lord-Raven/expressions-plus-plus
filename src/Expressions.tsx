...
type OutfitType = {
    ...
    images: EmotionPack;
    // Comma-delimited list of trigger words that will swap to this outfit
    triggerWords: string;
...
export class Expressions extends StageBase<InitStateType, ChatStateType, MessageStateType, ConfigType> {
            ...
            if (this.isSpeakerIdCharacterId(speakerId)) {
                try {
                    ...
                    if (response.errors && response.errors.length) {
                        console.log(`Failed sensitive storage access for ${speakerId}: ${response.errors}`);
                    } else {
                        console.log(`Successfully accessed sensitive storage for ${speakerId}`);
                        this.owns.push(speakerId);
                    }
                } catch (error) {
                    console.log(`Error accessing sensitive storage for ${speakerId}: ${error}`);
                }
            }
        }
    ...
    async updateEmotion(speaker: Speaker, content: string) {
        ...
        try {
            const emotionResult = await this.callPipeline(Pipeline.EMOTION, {param_0: content});
    ...
    async removeBackground(imageUrl: string, storageName: string) {
        ...
        try {
            ...
            const backgroundlessResponse = await this.callPipeline(Pipeline.REMOVE_BACKGROUND,{path: uploadResponse[0]});
    ...
    async generateBackgroundProperties(background: Background) {
        ...
        if (this.useBackgroundDepth) {
            try {
                ...
                const uploadResponse = await this.uploadHfBlob(Pipeline.REMOVE_BACKGROUND, imageBlob, 'backgroundDepth');
    ...
    async singleSpeakerCheck(speaker: Speaker) {
        ...
        try {
            const response = await this.callPipeline(Pipeline.ZERO_SHOT, {data_string: JSON.stringify({
